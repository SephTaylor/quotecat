...
- **Materials Margin Indicator (FREE, not Pro+ — corrected 2026-09-13)** - ✅ SHIPPED (Mar 2, 2026) - Shows materials margin alongside markup on quotes. User sets target in Business Settings, indicator shows green/yellow/red based on whether margin meets goal. Formula: margin% = markup% / (100 + markup%) × 100. Internal only - not shown on PDFs or client-facing exports. Key differentiator: competitors are just forms, QuoteCat helps send the *right* quote.
  - ⚠️ **This entry previously said Pro+. Wrong, corrected 2026-09-13 by Joseph.** Financial
    intelligence, including the margin work, is **in the FREE tier**. The genuinely Pro-gated
    analytics is the **Pricing Health Check** (`app/(main)/pricing-health-check.tsx`, checks
    `isPro` and presents the paywall otherwise).
  - ⛔ **`canAccessValueTracking` in `lib/features.ts` is DEAD CODE.** Defined, never called
    anywhere, gates nothing. Do not build paywall or marketing claims on it. It nearly put a
    false line on the paywall.
...
- **Job Photos (Before/After)** - Attach photos to quotes/invoices for job documentation. Before/after shots for marketing portfolio, progress photos for client updates, issue documentation for disputes. Photos stored in Supabase Storage, linked to quote/invoice IDs. Gallery view per job. Export for marketing use. Pairs well with Two-Way SMS for sending progress photos to clients. (Added Feb 25, 2026)

### Marketing (Premium Add-on)
- **Google Review Requests & Management** - Auto-request Google reviews after job completion or invoice paid. Send up to 2 follow-up reminders. Track which clients have reviewed (don't ask again). Dashboard showing average rating, total reviews, requests sent. **Review management:** See all reviews in-app, get notified of new reviews, quick-reply templates for responding (87% of people read reviews before hiring, and responding to reviews shows you care). Requires Google Business Profile API integration. Jobber charges $39/mo for this - could be Premium included or separate add-on. (Discussed Jan 17, 2026, expanded Feb 25, 2026)
...
### Growth
- **Regional Expansion Referral System** - When a user signs up and their zip code is outside covered regions (currently Michigan), show a "Launch QuoteCat in your area" screen instead of normal onboarding. **How it works:** (1) Generate unique referral code tied to user ID, store in Supabase; (2) User shares link `quotecat.ai/join?ref=CODE` with other contractors; (3) Track signups using referral code in `referrals` table; (4) When region hits 6 total Pro subscribers (referrer + 5), notify via Slack webhook and auto-extend referrer's subscription by 2 months; (5) Show referrer progress screen: "3 of 5 contractors joined". **Tables needed:** `regions` (zip ranges, coverage status), `referrals` (referrer_id, referee_id, region, created_at), `region_thresholds` (region, current_count, threshold, status). Join page at `quotecat.ai/join` works for both mobile and web. Great for organic expansion without marketing spend. (Discussed Mar 20, 2026)
