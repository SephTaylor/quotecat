...
- **Margin Goal Indicator** - Show margin alongside markup on every quote. User sets target margin once in settings. Each quote gets a simple indicator: ✓ green if it meets their goal, ⚠️ yellow/red if below. Already store markup, just need to calculate/display margin and compare to target. **Key differentiator:** Competitors (Jobber, Housecall Pro, ServiceTitan) are just forms - enter price, send. Nobody connects pricing education to the workflow. "QuoteCat doesn't just help you send quotes. It helps you send the right quotes." Low effort (~1 day), high value. (Discussed Feb 26, 2026)
