...
- **Onboarding wizard re-launches for users with complete settings (~30 min, JS-only, next release)** — `app/(main)/(tabs)/dashboard.tsx:127-131` decides whether to show the wizard based only on `prefs.onboarding?.completedAt` / `skippedAt`. A user signing in on a new device (or any user whose preferences were provisioned externally — e.g. via `lib/businessSettingsSync.ts` download) has complete `company`, `overhead`, and `pricing` blocks but no `completedAt` flag, so the wizard launches. Accepting the defaults then *overwrites* the real settings with zeros/empties and uploads them back to the cloud, corrupting both local AND cloud state. `components/OnboardingFlow.tsx:147-150` already auto-writes `completedAt` when steps are detected as complete, but that runs after the wizard has already rendered. Fix: in the dashboard mount check, gate `setShowOnboarding(true)` on actual data completeness — e.g. require missing `company.companyName` AND missing `overhead.annualOverhead` AND missing `pricing.defaultLaborRate` before launching. Surfaced 2026-06-29 while seeding screenshot data on `joseph@quotecat.ai`. ~30 min.
- **Labor Rate Calculator $ inputs missing thousands-separator formatting (~20 min, JS-only, next release)** — `app/(main)/labor-rate-calculator.tsx:178,194,226` placeholders show `"100,000"`, `"12,000"`, `"60,000"` with commas, but the inputs themselves run values through `filterNumber(v)` which strips non-digits and stores raw integers. Typed values display unformatted (`95000` instead of `95,000`), inconsistent with the placeholder's promise and harder to scan. Fix: wrap `value` with `value.replace(/\B(?=(\d{3})+(?!\d))/g, ',')` for display, keep raw stored in state, strip commas on next edit. Surfaced 2026-06-29 capturing screenshots. ~20 min.
